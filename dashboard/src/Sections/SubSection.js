...
const SubSection = ({ id, sub }) => {
  ...
  const deleteSub = (id) => {
    SectionsServices.deleteSection(`category/delete/${id}`).then(res => {
                ...
                {sub &&
                  sub.map((s, index) => (
                    <tr>
                      ...
                      <td>
                      <RiDeleteBinLine className={styles.Delete} onClick={()=>deleteSub(s._id)}/>
                      </td>
                    </tr>
                  ))}
              </tbody>
            </table> : <h5 className="text-danger">{i18n.language === 'en' ? 'There are no sub sections' : "لا يوجد أقسام فرعيه"}</h5>}
          </div>
