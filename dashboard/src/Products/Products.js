...
const Products = () => {
  ...
  // useEffect(()=>{
  //   SectionsServices.allSections(`category/getAllSub/${}`).then((res) => {
  //     setSubs(res.subCategories)
  //   });
  // },[])
  const getProductFilter = (option = undefined) => {
    ProductServices.getAll(`product/getAll/${option ? option : ""}`).then(
      (res) => {
        dispatch(getAll(res.products.products));
        SectionsServices.allSections(`category/getAllSub/${option}`).then((res) => {
          setSubs(res.subCategories)
      });
...
const getSubsFilter = (option = undefined) => {
  ProductServices.getAll(`product/getAll?sub=${option ? option : ""}`).then(
    (res) => {
      dispatch(getAll(res.products.products));
...
// END of subs filter
  ...
  const deleteHandler = (product) => {
    ProductServices.deleteProduct(`product/delete/${product._id}`).then(
      (_) => {
  ...
  const mostSellingHandler = () => {
    ProductServices.getAll(`product/getMostSelling`).then((res) => {
        ...
        <div className="row p-3 d-flex justify-content-lg-between justify-content-lg-between flex-md-row flex-column w-100">
          ...
          <div className=" d-flex flex-lg-row w-auto flex-column">
            ...
            <div className="dropdown m-1">
              ...
              <Link to='/products/addProduct' className="btn btn-warning text-white mx-3 ">
        ...
        <div className="row d-flex justify-content-lg-start justify-content-center p-2">
          {state !== ""
            ? copy.length > 0 ?
                ...
                >
                  <div className={styles.Hover}>
                    <div className="d-flex bg-light flex-column" style={{}}>
                      <Link to={`/products/edit/${product._id}`}><TbEdit className="my-1 rounded-circle bg-white text-primary fs-6" /></Link>
                      <AiFillDelete style={{ cursor: 'pointer' }} onClick={() => deleteHandler(product)} className="my-1 rounded-circle bg-white text-danger fs-6" />
                      <Link to={`/products/${product._id}`}><IoIosEye className="my-1 rounded-circle bg-white text-info w-100 fs-6" /></Link>
                    </div>
                  </div>
                  <div className="card w-0">
                    <Link to={`/products/${product._id}`} style={{ textDecoration: 'none', color: 'black', width: '100%', height: '100%' }}  >
                      <img
              ...
              )) : <h3 className="fs-3 text-danger text-center">{i18n.language === 'en' ? err.error_en : err.error_ar}</h3>
            : products.length > 0 ? products.map((product) => (
              ...
              >
                <div className={styles.Hover}>
                  <div className="d-flex bg-light flex-column" style={{}}>
                    <Link to={`/products/edit/${product._id}`}><TbEdit className="my-1 text-primary fs-6" /></Link>
                    <AiFillDelete style={{ cursor: 'pointer' }} onClick={() => deleteHandler(product)} className="my-1 text-danger fs-6" />
                    <Link to={`/products/${product._id}`}><IoIosEye className="my-1 text-info w-100 fs-6" /></Link>
                  </div>
                </div>
                <div className="card">
                  <Link to={`/products/${product._id}`} style={{ textDecoration: 'none', color: 'black', width: '100%', height: '100%' }}  >
