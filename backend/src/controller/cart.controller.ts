...
export const addToCart = asyncHandler(async (req: AuthenticatedRequest, res: Response
    ...
    if (foundUser && foundUser._id) {
        ...
        const isProductInCart = await Cart.findOne({ user: foundUser._id, product: req.body.product })
        if (isProductInCart) {
            const productExistinCart = await Cart.findByIdAndDelete(isProductInCart._id)
...
export const updateQuantityInCart = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const foundUser = req.user;
    ...
    if (foundUser) {
        ...
        if (req.body.Quantity) {
            ...
            updateQuantity = await Cart.findOneAndUpdate({ user: foundUser._id, product: req.body.product }, {
...
export const deleteFromCart = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const foundUser = req.user
    if (foundUser && foundUser._id) {

        const cart = await Cart.findOneAndDelete({ user: foundUser, product: req.params.productId })
        if (!cart) {
...
export const getCartWithUser = asyncHandler(async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {

    const foundUser = req.user
    const carts = await Cart.find({})
    if (foundUser && carts[0]) {

        const productsInCart = await Cart.find({ user: foundUser._id }).populate('product')
        if (!productsInCart[0]) {
...
export const deleteCartByUser = (asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const foundUser: any = await User.findById(req.user._id)

    const cart = await Cart.findOneAndDelete({ user: foundUser._id })
    if (!cart) {
